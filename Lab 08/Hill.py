...
def hillClimbing():
    ...
    parentCost = math.sqrt((graph[goalState].heuristic[0] - graph[initialState].heuristic[0])
                           ** 2 + (graph[goalState].heuristic[1] - graph[initialState].heuristic[1]))
    explored = []
    solution = []
    ...
    while parentNode != goalState:
        ...
        for child in graph[parentNode].actions:
            if child[0] not in explored:
                childCost = math.sqrt((graph[child[0]].heuristic[0] - graph[goalState].heuristic[0])
                                      ** 2 + (graph[child[0]].heuristic[1] - graph[goalState].heuristic[1]))
